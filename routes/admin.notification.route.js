...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      const recipientType = (req.body?.recipientType || "").trim().toLowerCase();
      ...
      if (recipients.length === 0) {
        return res
          .status(200)
          .json({ success: true, totalRecipients: 0, attempted: 0, sent: 0, failed: 0, previewUrls: [] });
      }
      ...
      for (const r of slice) {
        ...
        try {
          ...
          if (test && previewUrls.length < 20) {
            ...
            if (url) previewUrls.push({ userId: r.user_id, email: r.email, previewUrl: url });
          }
        } catch (e) {
          failed += 1;
          if (errors.length < 20) {
            errors.push({ userId: r.user_id, email: r.email, error: e.message || "send failed" });
          }
        }
      }
