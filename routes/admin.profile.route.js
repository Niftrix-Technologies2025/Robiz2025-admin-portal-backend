...
function toAbs(url) {
    ...
    return base
        ? base.replace(/\/$/, "") + "/" + String(url).replace(/^\//, "")
        : url;
}
...
async function buildProfile(userId) {
    ...
    const dp = toAbs(d.profile_picture) || toAbs(p.profile_image) || null;
    ...
    return {
        ...
        profile: {
            ...
            district:
                p.district_name || (u.district_id ? String(u.district_id) : ""),
...
router.post("/users/suspend-user", upload.none(), async (req, res) => {
    try {
        ...
        if (upd.rowCount === 1) {
            return res
                .status(200)
                .json({ success: true, userId, status: "SUSPENDED" });
        }
